...
while(True):
    if tbl.getBoolean("driveCam", False) == True and cs_bool == True:
        drive_ret, drive_img = drive_cap.read()
        outputStream.putFrame(drive_img)
    elif tbl.getBoolean("driveCam", False) == False:
        # Loop bools
        t_error = False
        i_error = False
        target_found = False
        ...
        bw_img = cv2.inRange(
            hls_img, (hue[0], lum[0], sat[0]),  (hue[1], lum[1], sat[1]))
        ...
        _, contours, hierarchy = cv2.findContours(
            img_closing, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        ...
        for c in contours:
            ...
            if w > 30 and w < 300 and h > 10 and h < 150:
                ...
                cv2.rectangle(img_to_push, (x, y), (x+w, y+h),
                              (0, 255, 0), 2)  # Thank you moth
                cv2.circle(img_to_push, (int(x+(w/2)),
                                         int(y+(h/2))), 5, (255, 0, 0))
                target = [int(x+(w/2)), int(y+(h/2))]

                cv2.line(img_to_push, (target[0]-10, target[1]),
                         (target[0]+10, target[1]), (255, 255, 255))
                cv2.line(img_to_push, (target[0], target[1]-10),
                ...
                fin_angle_hori = (
                    (math.atan((target[0]-(img_w/2))/flength)))*(180/math.pi)

                # Calculate vertical angle for distance calculations
                fin_angle_vert = (
                    (((math.atan((img_h/2)-target[1]))*-1)/flength)*(180/math.pi)) + cam_offset
                distance = target_height / \
                    math.tan(fin_angle_vert*(math.pi/180))
                target_found = True
        ...
        if target_found:
            print("Turn to ", fin_angle_hori, " Distance:", distance,
                  " Calculated angle:", fin_angle_vert, " (X, Y):", target[0], target[1])
