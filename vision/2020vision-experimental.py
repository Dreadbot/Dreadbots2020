...
#Set selectedCam string
tbl.putString("selectedCam", 'vision')

#Get feed from camera/img
vision_cap = cv2.VideoCapture(0)
geneva_cap = cv2.VideoCapture(2)
floor_cap = cv2.VideoCapture(1)
drive_cap = cv2.VideoCapture(3)
...
def resImage(img, factor):
    scale_factor = factor

    width = int(img.shape[1] * scale_factor / 100)
    height = int(imgimg.shape[0] * scale_factor / 100)
    dim = (width, height)
...
while(True):
    ...
    if cs_bool:
        if tbl.getString("driveCam", 'vision') == 'vision':
            outputStream.putFrame(resImg(img_to_push, 50))

        if tbl.getString("driveCam", 'vision') == 'drive':
            drive_img = drive_cap.read()
            outputStream.putFrame(resImg(drive_img, 50))

        if tbl.getString("driveCam", 'vision') == 'geneva':
            geneva_img = geneva_cap.read()
            outputStream.putFrame(resImg(geneva_img, 50))
            
        if tbl.getString("driveCam", 'vision') == 'floor':
            floor_img = floor_cap.read()
            outputStream.putFrame(resImg(floor_img, 50))
